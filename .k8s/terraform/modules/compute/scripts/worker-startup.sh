#!/bin/bash
# K3s Worker Node Startup Script
# This script runs on first boot to prepare the VM for K3s installation
# Actual K3s worker joining is done via the install-k3s-worker.sh script

set -euo pipefail

# Logging
exec > >(tee -a /var/log/worker-startup.log)
exec 2>&1

echo "=========================================="
echo "K3s Worker Node Preparation"
...
# Create environment file for K3s worker configuration
...
touch /var/log/worker-prepared
echo "Worker node preparation completed at: $(date)" >> /var/log/worker-prepared
...
echo "2. Run the K3s worker join script from: $SETUP_DIR"
echo ""
