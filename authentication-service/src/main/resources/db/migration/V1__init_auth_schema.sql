...
COMMENT ON COLUMN auth_oltp.users.email IS 'User email address, unique and optional, used for profile updates and notifications';
COMMENT ON COLUMN auth_oltp.refresh_tokens.created_at IS 'Timestamp when the refresh token was created';
COMMENT ON COLUMN auth_oltp.users.created_by IS 'Identifier for the user/system that created the record';
COMMENT ON COLUMN auth_oltp.users.updated_by IS 'Identifier for the user/system that last updated the record';
...
CREATE OR REPLACE FUNCTION auth_oltp.cleanup_old_tokens()
    RETURNS INTEGER AS
$$
DECLARE
...
COMMENT ON FUNCTION auth_oltp.cleanup_old_tokens() IS 'Permanently remove revoked refresh tokens older than 30 days.';
